class BookingsController < ApplicationController
  ...
  def create
    ...
    if @booking.save
      ...
      @booking_notification = PersonalMessage.create(body: "A new booking request has been made at #{@space.title}: from #{@booking.start_time.in_time_zone("Eastern Time (US & Canada)").strftime("%-d %b %Y")} to #{@booking.end_time.in_time_zone("Eastern Time (US & Canada)").strftime("%-d %b %Y")}.", conversation_id: @conversation.id, user_id: current_user.id)
  ...
  private
  def booking_params
    params.require(:booking).permit(:start_time, :end_time, :note, :user_id, :approved, :denied)
  end
